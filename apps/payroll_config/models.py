...
class LegalParameter(models.Model):
    ...
    overtime_base = models.CharField(
        ...
        choices=[
            ...
            ("MONTHLY_BASE_30", "(مزد روزانه + پایه سنوات روزانه) × ۳۰ ÷ ساعات کار ماهانه"),
        ],
        help_text="حالت دوم: (مزد روزانه × ۳۰ + پایه سنوات روزانه × کارکرد) ÷ ۲۲۰ — "
                  "حالت سوم پایهٔ سنوات را هم در ۳۰ ضرب می‌کند، نه در کارکرد",
    )
    # تسهیم مزایای ثابت ماهانه (مسکن، بن، اولاد، تأهل) وقتی کارکرد ناقص است.
    #
    # فایل ۱۴۰۵ شرکت مخرج را **۳۱ ثابت** می‌گیرد و کارکرد ۳۰ روز را هم کاملِ
    # ماه حساب می‌کند:
    #     =IF(OR(O=30,O=31), مبلغ, مبلغ/31*O)
    #
    # در تیر و مرداد هر دو روش یک جواب می‌دهند، چون هر دو ماه ۳۱ روزه‌اند و
    # هیچ‌کس دقیقاً ۳۰ روز کارکرد ندارد. از **مهر** به بعد (ماه‌های ۳۰ روزه)
    # این دو از هم جدا می‌شوند و برای هر کارکرد ناقص، اختلافشان میلیونی است.
    allowance_proration = models.CharField(
        "تسهیم مزایای ثابت", max_length=16, default="MONTH_DAYS",
        choices=[
            ("MONTH_DAYS", "کارکرد قابل پرداخت ÷ روزهای ماه کاری (توصیه‌شده)"),
            ("FIXED_31", "مخرج ثابت ۳۱ — عیناً فرمول فایل ۱۴۰۵"),
            ("MONTH_30", "ماهِ کامل → کامل؛ ناقص → کارکرد ÷ ۳۰"),
        ],
        help_text="حالت اول: ماهِ کامل همیشه کامل پرداخت می‌شود — چه ۲۹ روزه چه ۳۱ "
                  "روزه — و فقط غیبت و بیماری و مرخصی بدون حقوق کمش می‌کنند. "
                  "«روزهای ماه کاری» هر دوره در ساخت همان دوره تعیین می‌شود.",
    )
...
class SalaryComponent(models.Model):
    ...
    # این مکانیزم است.
    # آیا ستون «تعداد» روی فیش برای این قلم عدد نشان بدهد؟
    #
    # اقلامی مثل حق مسکن و بن و تأهل و پایه سنوات، تعدادشان روزهای کارکرد ماه
    # است — عددی که خواننده فیش آن را «تعداد» نمی‌فهمد و فقط شلوغش می‌کند.
    # اضافه‌کاری و مأموریت برعکس: ساعت و روزشان خودِ اطلاعات است.
    #
    # داده است نه کد، چون کدام قلم عدد لازم دارد از شرکتی به شرکتی فرق می‌کند.
    show_quantity = models.BooleanField(
        "نمایش تعداد روی فیش", default=True,
        help_text="برای اقلامی که تعدادشان روزهای ماه است، خاموش کنید",
    )
    ...
    # قرینهٔ `absorbs`. فرمول فایل شرکت برای پورسانت فروش دو طرف دارد و تا
    # امروز فقط یک طرفش داده بود:
    #
    #     پورسانت قابل پرداخت = (پورسانت خام + افزوده‌شونده‌ها) − (جذب‌شونده‌ها)
    #
    # «مازاد ثابت» تا امروز از راه `allocation_role` اضافه می‌شد، ولی آن فیلد
    # برای زنجیرهٔ مازاد است نه برای این فرمول. «وجه مرخصی» هم در فرمول فایل
    # اضافه می‌شود و هیچ نقشی در زنجیرهٔ مازاد ندارد — بدون این فیلد، جایی
    # برای بیانش نبود.
    adds = models.ManyToManyField(
        "self", symmetrical=False, blank=True, related_name="added_to",
        verbose_name="اقلامی که به این قلم اضافه می‌شوند",
        help_text="مبلغ این اقلام پیش از کسر جذب‌شونده‌ها به این قلم اضافه می‌شود",
    )
