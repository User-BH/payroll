...
def leave_pay_monthly(ctx: PayrollContext, component):
    ...
    و آن سهمیه خودش از تاریخ استخدام ساخته می‌شود:

        سهمیه = ROUNDUP( ۲۶ × روزهای اشتغال در سال ÷ ۳۶۵ )

    مگر برای کسی که سهمیه‌اش صریحاً «دستی» علامت خورده باشد.
    """
...
def commission_net(ctx: PayrollContext, component):
    ...
    **چرا جذب، نه کسور؟** کسور از خالص کم می‌شود و ماخذ بیمه و مالیات را دست
    ...
    فرمول کاملِ ستون `AR` در فایل ۱۴۰۵:

        (پورسانت یک + مازاد ثابت + وجه مرخصی)
      − (مابه‌التفاوت سنوات + ذخیره پورسانت + کسر مازاد پرداختی فروش + مبلغ مأموریت)

    دو قلمِ «وجه مرخصی» و «کسر مازاد پرداختی فروش» اینجا مدل نشده‌اند، چون در
    هر ۱۴ شیتِ هر دو فایل (تبریز و اردبیل) برای **همهٔ** سطرهای فروش صفرند.
    اگر روزی ناصفر شدند، باید به `absorbs` اضافه شوند.

    **منفی شدن.** نتیجه می‌تواند منفی شود و عمداً صفر نمی‌شود؛ صفر کردنش یعنی
    پنهان کردن اینکه جذب از خودِ پورسانت بیشتر بوده. ولی در عمل تقریباً هرگز
    منفی نمی‌شود، و دلیلش روشن است:

        تبریز  ۱۴۱ نفر-ماهِ فروش با پورسانت → صفر مورد منفی
        اردبیل  یک مورد: اردیبهشت، علی قاسمی ایرلو، ۹۸۰٬۸۴۳− ریال

    علتش این است که حسابدار خودش **تهاتر** می‌کند: در ۸ سطر از آن ۱۴۱ سطر،
    نتیجه دقیقاً صفر است و در هر هشت‌تا یک تساوی برقرار است —

        پورسانت یک  =  مابه‌التفاوت پایه سنوات تجمیعی

    یعنی ستون پورسانت فقط «کارکرد فروش» نیست؛ محلِ تأمینِ مابه‌التفاوت سنوات
    هم هست. وقتی کسی مابه‌التفاوت دارد، پورسانتش به همان اندازه وارد می‌شود و
    دو عدد همدیگر را خنثی می‌کنند. پس مبلغی که وارد می‌شود هیچ‌وقت از جذب
    کمتر نیست.

    **و چرا با «تخصیص خودکار پورسانت» منفی می‌شد.** در فایل، «مبلغ مأموریت»
    یک ورودی مستقل است که از پورسانت کم می‌شود — یک حرکت. تخصیص خودکار حرکت
    دومی اضافه می‌کند: بخشی از پورسانت را **به** مأموریت تبدیل می‌کند. آن‌وقت
    مأموریت بالا می‌رود و همان مبلغِ بالارفته دوباره از پورسانت جذب می‌شود.
    یعنی یک ریال دو بار برداشته می‌شود، و تهاترِ حسابدار که برای یک حرکت
    تنظیم شده بود می‌شکند. در مرداد ۱۴۰۵ همین ۱۴ فیش از ۶۸ فیش را خراب کرد.
    """
