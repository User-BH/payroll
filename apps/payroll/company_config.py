...
def configure(company, stdout=None):
    ...
    # تبدیل خودکار پورسانت به مأموریت — **روشن**، انتخابِ اعلام‌شدهٔ شرکت.
    #
    # ماندهٔ پورسانت پس از جذب، تا سقف روز مأموریت به حق مأموریت تبدیل می‌شود
    # و بقیه‌اش روی سطر پورسانت می‌ماند.
    #
    # این با فایل ۱۴۰۵ فرق دارد و باید دانسته شود: در آن فایل روز مأموریتِ
    # فروش **دستی وارد می‌شود** و پورسانت رقمِ متعادل‌کننده است (سند
    ...
    if turned:
        say(
            ...
            + " — ماندهٔ پورسانت تا سقف روز مأموریت به حق مأموریت می‌رود."
        )
