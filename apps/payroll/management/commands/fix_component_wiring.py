...
WIRING = {
    ...
    "FOOD": "food_allowance",
    "SENIORITY": "seniority",
    # حق تأهل هم همین ایراد را داشت، با یک اشکال اضافه: حتی اگر مبلغ ثابتش پر
    # می‌شد، به همه می‌رسید — چون هیچ‌جا وضعیت تأهل خوانده نمی‌شد.
    "MARRIAGE": "marriage_allowance",
}
        ...
        for code, rule_key in WIRING.items():
            for component in SalaryComponent.objects.filter(code=code):
                ...
                if component.fixed_amount:
                    ...
                    if code == "MARRIAGE":
                        # این یکی فرق دارد: «مبلغ ثابت» یعنی به **همه** می‌رسد،
                        # چه متأهل چه مجرد — چون هیچ‌جا وضعیت تأهل خوانده
                        # نمی‌شود. سکوت کردن اینجا یعنی رها کردن یک پرداخت غلط.
                        self.stdout.write(self.style.WARNING(
                            "      هشدار: با مبلغ ثابت، حق تأهل به مجردها هم پرداخت می‌شود.\n"
                            "      مبلغ را در «تنظیمات سال مالی ← حق تأهل ماهانه» بگذارید و\n"
                            "      مبلغ ثابت این قلم را صفر کنید، بعد دوباره این دستور را بزنید."
                        ))
                    continue
