...
def _to_decimal(value):
    ...
    فعالی ندارد و جنگو `current_contract.daily_wage` را به "" تبدیل می‌کند.
...
def fa_wage(value, persian=True) -> str:
    """نرخ واحد (مزد روزانه، مزد ساعتی) — با اعشار، اگر اعشار داشته باشد.

    مبلغ نهایی هر سطر همیشه ریال صحیح است، ولی **نرخ** اغلب نیست: مزد روزانه
    یعنی حقوق ماه تقسیم بر ۳۱، و مزد ساعتی یعنی همان تقسیم بر ۷٫۳۳. اگر نرخ را
    در توضیح گرد کنیم، خواننده‌ای که با ماشین‌حساب همان ضرب را انجام می‌دهد به
    عددی چند ریال متفاوت با سطر می‌رسد و به کل فیش شک می‌کند.

    پس موتور با نرخ دقیق حساب می‌کند و **یک بار در آخر** گرد می‌کند، و توضیح هم
    نرخ را با سه رقم اعشار نشان می‌دهد. سه رقم را با اندازه‌گیری انتخاب کردیم:
    روی ۲۹۲ سطر حقوق ثابت، با یک رقم ۱۱۳ سطر و با دو رقم ۳ سطر با ماشین‌حساب
    درنمی‌آمد و با سه رقم هیچ سطری. چون مزد روزانه کسر متناوب است، در حالت‌های
    نادری که حاصل‌ضرب دقیقاً روی مرز نیم‌ریال بیفتد باز هم ممکن است **یک ریال**
    اختلاف بماند؛ این باقی‌ماندهٔ تقسیم است، نه خطای محاسبه.

    اعشار صفر حذف می‌شود، پس نرخ رُندی مثل حداقل دستمزد همان‌طور صحیح می‌ماند.
    """
    number = _to_decimal(value)
    if number is None:
        return "—"
    return fa_number(number, 3, persian=persian)
