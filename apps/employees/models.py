...
class Employee(models.Model):
    ...
    # آیا پورسانتِ این نفر به حق مأموریت و اضافه‌کاری تبدیل شود؟
    #
    # قاعدهٔ گروه فروش «بله» است و پیش‌فرض هم همان است، ولی استثنا واقعی
    # وجود دارد و باید **روی خودِ شخص** بنشیند نه روی یک ماه: اگر استثنا را
    # با «تخصیص دستیِ صفر» در یک دوره بسازیم، ماه بعد دوباره تبدیل می‌شود و
    # کسی خبردار نمی‌شود. با این کلید، تصمیم یک بار گرفته می‌شود و می‌ماند.
    #
    # خاموش که باشد هیچ تخصیصی ساخته نمی‌شود، پس نه سطر «حق ماموریت فروش»
    # می‌آید و نه چیزی از پورسانت کم می‌شود؛ کلِ پورسانتِ خالص در سطر
    # پورسانت یک می‌ماند.
    commission_to_mission = models.BooleanField(
        "تبدیل پورسانت به مأموریت و اضافه‌کاری", default=True,
        help_text="خاموش کنید تا پورسانت این نفر تبدیل نشود و کامل در سطر "
                  "پورسانت بماند",
    )
